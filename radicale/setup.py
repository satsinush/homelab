...
def compile_radicale_configs(env: dict, users: list[dict]) -> None:
    ...
    # 4. Write rights file to permit full read/write for authenticated users
    rights_content = """# Generated by radicale/setup.py
[allow_authenticated]
user: .+
collection: .+
permission: rw
"""
    ...
    with open(rights_path, "w", encoding="utf-8") as f:
        f.write(rights_content)
    os.chmod(rights_path, 0o600)
    run_cmd(f"sudo chown {_RADICALE_UID}:{_RADICALE_UID} {rights_path}")
